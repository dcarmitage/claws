...
claws is a CLI tool (v2.0.0a4) for building, training, and managing AI agents. Agents have identity, memory, and evaluation history. The codebase is a Python package (`pip install claws`) built with Click, Rich, and httpx.
...
│   │   └── openai_compat.py # OpenAI-compatible (OpenRouter, OpenAI, etc.)
...
├── docs/                   # Architecture, best practices, philosophy
│   └── philosophy/         # Archived design philosophy docs
...
- **Python:** 3.8+. `from __future__ import annotations`. Type hints welcome.
...
- **Events:** Use the event log (`EventSpine.emit()`) for all state changes. Never modify events.jsonl directly.
...
- Don't add dependencies without checking cross-platform availability
