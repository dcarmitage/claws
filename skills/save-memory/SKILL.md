...
```bash
# What changed today?
git -C $CLAWS_HOME log --oneline --since="midnight" 2>/dev/null || echo "no commits today"

# What files are new or modified?
git -C $CLAWS_HOME status --short 2>/dev/null
...
### 2. Update Daily Memory (`$CLAWS_HOME/memory/YYYY-MM-DD.md`)
...
### 3. Update MEMORY.md (`$CLAWS_HOME/MEMORY.md`)
...
### 4. Update Auto Memory (if running as your-user)

Update `$HOME/.claude/projects/-home-your-user/memory/MEMORY.md` with:
...
wc -l $CLAWS_HOME/memory/$(date +%Y-%m-%d).md

# MEMORY.md was updated recently
stat -c '%Y %n' $CLAWS_HOME/MEMORY.md 2>/dev/null || stat -f '%m %N' $CLAWS_HOME/MEMORY.md

# No unstaged important files
git -C $CLAWS_HOME status --short | grep -v '^\?\?' | head -10
```
