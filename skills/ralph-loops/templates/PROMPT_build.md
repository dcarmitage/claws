...
4. When the tests pass, update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a message describing the changes. After the commit, run the dual-judge evaluation:
   ```bash
   bash $CLAWS_HOME/evals/run_dual_judge_eval.sh \
...
9999999999999999999. JUDGE GATE: Never push a commit that hasn't passed both dual judges (logic + consistency) with scores >= 8.0. The judges live at `$CLAWS_HOME/evals/`. If a judge fails, the failure output tells you exactly which claims or checks failed — fix the root cause, don't retry blindly. This is backpressure, not bureaucracy.
