...
1. **Resolve the commit.** If `$ARGUMENTS` contains a SHA, use it. Otherwise use HEAD:
   ```bash
   git -C $CLAWS_HOME log --oneline -1 HEAD
   ```
...
4. **Get the auth token:**
   ```bash
   OPENCLAW_TOKEN=$(python3 -c "import json; print(json.load(open('$OPENCLAW_CONFIG'))['gateway']['auth']['token'])")
   ```

5. **Run both judges in parallel:**
   ```bash
   # Save diff to temp file (process substitution doesn't work reliably)
   git -C $CLAWS_HOME show <commit> --format="" > /tmp/judge_diff.txt

   OPENCLAW_TOKEN=$OPENCLAW_TOKEN bash $CLAWS_HOME/evals/logic_judge.sh \
     --spec <spec_path> \
     --diff /tmp/judge_diff.txt \
     --test-output "<any test output from the build, or describe what was verified>"

   OPENCLAW_TOKEN=$OPENCLAW_TOKEN bash $CLAWS_HOME/evals/consistency_judge.sh \
     --spec <spec_path> \
     --taskboard <taskboard_path> \
     --diff /tmp/judge_diff.txt \
     --changed-files "$(git -C $CLAWS_HOME show <commit> --name-only --format=""  | tr '\n' ',')"
   ```
