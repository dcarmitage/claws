...
3. Updated best practices and evals (how to improve)
...
Context corruption doesn't degrade gradually — it hits a cliff (coherent to garbage). The sub-agent doing 6 tasks didn't fail on task 1; it failed on the cumulative weight.
...
Server endpoints, then time display, then mic icon, then audio hint, then scrubber. Each task's output was the next task's input. The bulk approach failed because tasks stepped on each other.
...
- **Learning:** Updated best practices, new anti-patterns, refined templates

*Action: After every build, generate a report, update case studies, sync to shared learning. Git snapshot everything.*
...
**Every task gets two-pass evaluation after tests pass.**

Automated tests catch syntax and functional errors. Judges catch semantic drift — does the code actually fulfill the spec's intent? Tests passing is necessary but not sufficient. The two-pass gate catches what tests miss: scope creep, spec misalignment, cross-file inconsistency.

*Action: After task completion with tests passing, run the two-pass evaluation before advancing to the next task. Both judges must score >= 8.0. No exceptions without documented root cause.*
...
*Action: After each build, review judge scores in metrics. If average scores decline session-over-session, investigate before the next build. Common causes: stale specs, scope creep, context degradation (H3).*
...
1. *Read the full judge output (saved in eval results)*
...
1. Check if it fits an existing practice — if so, add evidence/examples
2. If it's a new pattern, add a new H# or E# entry
3. If it contradicts an existing practice, update it with the new understanding
...
On 2026-02-01, the main session hit ~85% context and output degraded to token garbage. The agent did not notice or warn the human. The human had to ask "how much context do you have left?" twice.
...
**Code existing does not mean code working. You don't stop until the thing runs, you've tested it yourself, and you can prove it works. Never fabricate, assume, or extrapolate test results.**

On 2026-02-01, plugin files were created by a sub-agent but never installed, configured, or tested. They got swept into a `git add -A` commit alongside real documentation — implicitly presenting unverified code as completed work. When asked, the initial response was defensive before actually checking.

This is a trust violation. The human cannot make decisions based on false information about what works.
...
Evidence: The human caught it. Trust is the foundation. This practice is non-negotiable.
...
1. **Convergence = priority signal.** All 5 converged on the same first action despite different mandates. When independent perspectives agree unprompted, that's gravity, not coincidence.
2. **Diverse lenses catch different things.** The Infrastructure agent caught 17 uncommitted files (urgent data loss risk) that every strategy-focused agent missed. Urgency does not equal importance, and no single lens sees both.
...
### H15: Simulation Does Not Equal Production — Close the Gap Fast
**A system tested only in simulation is a prototype, not a product.**

On 2026-02-05, the two-pass evaluation system was built and "verified working" with synthetic test cases. But all 5 strategy council agents independently flagged the same gap: it has never fired in a real build session. The hook, the token discovery, the LLM response parsing, the build log integration — all tested individually, never as a connected system under real conditions.
...
### H12: Checkpoint After Every Milestone
...
On 2026-02-05, a fix commit changed only ~8 lines. When judges re-evaluated, they couldn't verify the full spec against such a small diff — 12 of 14 claims became UNVERIFIABLE, scores cratered. Squashing the fix into the original commit (so judges see the complete implementation) immediately passed.

*Action: When a judge fails and you fix the issue, squash the fix into the original commit before re-running judges. Never re-evaluate against an incremental patch.*

### H17: Verify Before Assigning
**Re-check the problem before sending an agent to fix it. Stale intel wastes agent work.**

On 2026-02-05, the strategy council flagged "17 modified + 17 untracked files" as urgent. By the time Agent 2 was dispatched to commit them, they were already committed. Agent 2 found only 1 file to commit — the mission was 95% redundant.
