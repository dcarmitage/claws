# Infrastructure Scout Report: Raspberry Pi 5 (agent-alpha) + Pi 4 (agent-beta)

**Date:** 2026-02-06
**Audited by:** Infrastructure Scout Agent
**Systems:** agent-alpha (Pi 5, <your-agent-ip>), agent-beta (Pi 4, SSH alias `agent-beta`)
...
| Property | Value |
|---|---|
| **User** | `your-user` (uid=1000, gid=1000) |
...
| **clawd user** | **Does NOT exist** on agent-alpha. `$CLAWS_HOME/` is owned by `your-user:your-user` (mode 755) — it's just a directory, not a separate user account |
| **Shell** | /bin/bash |
| **Home** | $HOME (mode 700) |
...
| **Gateway** | <your-network-ip> |
| **DNS** | <your-network-ip> (via NetworkManager) |
| **Ethernet** | eth0 present but NO-CARRIER (cable not connected) |

### Open Ports (agent-alpha)
...
| **Config dir** | `$HOME/.openclaw/` (mode 700) |
| **Workspace** | `$HOME/.openclaw/workspace/` |
| **Agent home** | `$CLAWS_HOME/` (the operational workspace) |
...
| Service | Description |
|---|---|
| camservice.service | Picamera2 camera service (`$CLAWS_HOME/tools/camservice.py`) |
...
| **AgentChat** | Enabled | Server at 127.0.0.1:9090, agent name "agent-alpha" |
...
|---|---|---|
| Read/write files in $HOME/ | ✅ YES | Home dir is mode 700, but agent runs as your-user |
| Read/write files in $CLAWS_HOME/ | ✅ YES | Owned by your-user, mode 755 |
...
| SSH to agent-beta | ✅ YES | Key-based auth configured |
...
**However:** your-user has `NOPASSWD: ALL` sudo, so effectively the agent CAN do anything with `sudo`. The boundary is "will the agent choose to use sudo" not "can it."
...
| File | Purpose |
|---|---|
| `~/.ssh/id_ed25519` | your-user's SSH key |
| `~/.ssh/authorized_keys` | 2 keys: your-user@gmail.com + agent-beta@agent-beta |
| `$CLAWS_HOME/.ssh/agentchat_deploy` | Deployment key for AgentChat |
| No `~/.ssh/config` | SSH aliases must use /etc/hosts or mDNS |

agent-beta can SSH into agent-alpha (authorized key present). agent-alpha can SSH into agent-beta (tested and working).

### File Permission Map

```
$HOME/     700 your-user:your-user  (private)
$HOME/.openclaw/  700 your-user:your-user  (private, contains tokens)
$CLAWS_HOME/          755 your-user:your-user  (world-readable, writable by agent)
$CLAWS_HOME/.ssh/     700 your-user:your-user  (private)
$CLAWS_HOME/.claude/  (exists, owned by your-user)
/tmp/                 world-writable
```
...
### agent-alpha: Raspberry Pi 5 Model B Rev 1.1
...
### agent-beta: Raspberry Pi 4 (via SSH)
...
### Key Difference: agent-beta's gateway binds to localhost only (not LAN), while agent-alpha's binds to all interfaces.
...
|---|---|---|
| **Bash/Shell execution** | ✅ | Full shell access as your-user |
...
| `sudo apt install` | ✅ | your-user has NOPASSWD sudo |
...
7. **Agent autonomy:** OpenClaw agent has the same permissions as your-user — effectively root.
...
```
[HUMAN PRE-CONFIG] SSH key for agent-beta access
  — Needed for multi-Pi orchestration
  — Already done: agent-beta key in ~/.ssh/authorized_keys
  — Command: ssh-copy-id agent-beta
...
[HUMAN PRE-CONFIG] GPIO/I2C/SPI group membership
  ...
  — Already done: your-user is in gpio, i2c, spi groups
...
[MISSING] Docker
  ...
  — Fix: sudo apt install docker.io && sudo usermod -aG docker your-user

[MISSING] Separate agent user
  — Agent runs as your-user with full sudo. No privilege separation.
...
[MISSING] Backup system
  — No automated backups of SD card or $CLAWS_HOME/
...
## 7. agent-beta Comparison

| Property | agent-alpha (Pi 5) | agent-beta (Pi 4) |
|---|---|---|
...
1. **Agent = Root.** The agent runs as your-user with NOPASSWD sudo. There is no privilege separation. Any security boundary is voluntary, not enforced.
...
6. **Two-Pi architecture is operational.** agent-alpha (muscle, 16GB, Hailo) and agent-beta (lightweight, 4GB) communicate via SSH. agent-alpha's gateway is LAN-accessible; agent-beta's is localhost-only.
