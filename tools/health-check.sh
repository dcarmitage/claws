...
check "SD card space >5GB free" "[ $(df /home/clawd --output=avail | tail -1) -gt 5000000 ]"
warn "USB drive space >5GB free" "[ $(df /mnt/media --output=avail 2>/dev/null | tail -1) -gt 5000000 ]"

echo ""
echo "Tools:"
check "git repo clean" "cd /home/clawd && git diff --quiet HEAD 2>/dev/null"
check "QMD installed" "test -f /home/clawd/tools/qmd/src/qmd.ts"
warn "QMD index exists" "test -f $HOME/.cache/qmd/index.sqlite"
warn "bun available" "which bun || test -f $HOME/.bun/bin/bun"

echo ""
echo "Knowledge:"
check "MEMORY.md exists" "test -f /home/clawd/MEMORY.md"
check "LEARN.md exists" "test -f /home/clawd/LEARN.md"
check "Today's memory exists" "test -f /home/clawd/memory/$(date +%Y-%m-%d).md"
