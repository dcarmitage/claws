#!/bin/bash
# Task Dispatch Poller — agent-beta runs this to watch for assigned tasks.
#
...
#
# Usage: AGENT=agent-beta ./dispatch-poller.sh
#
# Environment:
#   AGENT          - this agent's ID (default: agent-beta)
#   SERVER         - AgentChat server URL (default: http://<your-agent-ip>:9090)
...
AGENT="${AGENT:-agent-beta}"
SERVER="${SERVER:-http://<your-agent-ip>:9090}"
POLL_INTERVAL="${POLL_INTERVAL:-10}"
INBOX_FILE="${INBOX_FILE:-/tmp/task-dispatch-inbox.json}"
AUTO_CLAIM="${AUTO_CLAIM:-true}"
DISPATCH_PY="$CLAWS_HOME/tools/agentchat/task_dispatch.py"
