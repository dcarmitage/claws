...
DB_PATH = '$CLAWS_HOME/tools/agentchat/chat.db'
HTTP_PORT = 9090
WS_PORT = 9091
WEBHOOK_CONFIG = '$CLAWS_HOME/tools/agentchat/webhooks.json'
...
class AgentChatHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/' or path == '':
            try:
                with open('$CLAWS_HOME/tools/agentchat/dashboard.html', 'r') as f:
                    html = f.read()
                self.send_response(200)
    ...
    def do_POST(self):
        ...
        if path == '/api/send':
            ...
            text = data.get('text', '')
            # Map to v2
            sender_id = 'agent-alpha' if 'agent-alpha' in sender.lower() else 'agent-beta'
        ...
        if path.startswith('/api/v2/channels/') and '/messages' in path:
            channel_id = path.split('/')[4]
            sender_id = data.get('sender_id', 'agent-alpha')
            content = data.get('content', '')
...
╔═══════════════════════════════════════════╗
║         AgentChat V2 Server               ║
║   HTTP + WebSocket • Built on agent-alpha     ║
╚═══════════════════════════════════════════╝
    """)
