...
while true; do
    ...
    if mountpoint -q "$MEDIA_ROOT" 2>/dev/null; then
        ...
        FREE=$(df -h /home/clawd/media | awk 'NR==2{print $4}')
        TOTAL=$(df -h /home/clawd/media | awk 'NR==2{print $2}')
        PCT=$(df /home/clawd/media | awk 'NR==2{print $5}')
    fi
    ...
    echo "@dcarmitage  ${DATESTAMP} ${TIMESTAMP}" > "$TEXTFILE"
