...
*A shared knowledge base for all agents in your-human's ecosystem. Any agent that boots up should be able to read this and get oriented fast.*
...
- **World model** — Each agent boots up with a world model that initially mirrors your-human's. Over time, agents develop specialized perspectives while staying grounded in shared reality.
...
|--------|----------|------|--------|
| Raspberry Pi 5 (16GB) | `agent-alpha` / <your-agent-ip> | Media station, camera, mic, AI inference | 🟢 Active |
...
### Services Running on agent-alpha
...
|-------|------|------|--------|
| **agent-alpha** 🌀 | Raspberry Pi 5 | Media librarian, first teammate, infrastructure builder | 🟢 Active |
| *(Future agents)* | Various | Specialized roles (TBD via mitosis/specialization) | 🔴 Planned |

### your-human (Human)
- **Role:** Creator, collaborator, validator
- **Comms:** Telegram (@your-handle)
...
- **Agent ↔ Agent:** AgentChat (Convex-powered, REST API)
  ...
  - Credentials: `$CLAWS_HOME/.secrets/agentchat.json`
  - agent-alpha username: `moltbot_agent-alpha`
...
### Development Process: agent-alpha Build Method
...
```
1. DISCUSS    — Talk through what we're building with your-human
...
- **Web viewer:** `http://agent-alpha.local:5080/stream/watch`
...
*Last updated: 2026-02-01 by agent-alpha 🌀*

## The Armada

### Launch Process (Proven 2026-02-01)
1. Flash SD card (Pi Imager, OS Lite 64-bit, SSH + WiFi enabled)
2. Boot, find on network (`ping sweep <your-network-ip>/24`)
3. SSH key access from agent-alpha
...
### Armada Architecture
- **agent-alpha** (Pi 5, 8GB): Media librarian, Opus 4.5
- **agent-beta** (Pi 4, 4GB): Researcher (planned), GPT-5.2
